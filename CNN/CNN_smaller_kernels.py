...
# Define the VGG11 architecture with smaller kernels
class VGG11_SmallerKernels(nn.Module):
    def __init__(self):
        super(VGG11_SmallerKernels, self).__init__()
        self.features = nn.Sequential(
            nn.Conv2d(3, 64, kernel_size=2, stride=1, padding=1),  # Conv1 (2x2 kernel)
            nn.BatchNorm2d(64),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # Pool1

            nn.Conv2d(64, 128, kernel_size=2, stride=1, padding=1),  # Conv2 (2x2 kernel)
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # Pool2

            nn.Conv2d(128, 256, kernel_size=2, stride=1, padding=1),  # Conv3 (2x2 kernel)
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.Conv2d(256, 256, kernel_size=2, stride=1, padding=1),  # Conv4 (2x2 kernel)
            nn.BatchNorm2d(256),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # Pool3

            nn.Conv2d(256, 512, kernel_size=2, stride=1, padding=1),  # Conv5 (2x2 kernel)
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.Conv2d(512, 512, kernel_size=2, stride=1, padding=1),  # Conv6 (2x2 kernel)
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2, stride=2),  # Pool4

            nn.Conv2d(512, 512, kernel_size=2, stride=1, padding=1),  # Conv7 (2x2 kernel)
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.Conv2d(512, 512, kernel_size=2, stride=1, padding=1),  # Conv8 (2x2 kernel)
            nn.BatchNorm2d(512),
            nn.ReLU(),
...
cnn_model = VGG11_SmallerKernels().to(device)
...
torch.save(cnn_model.state_dict(), "CNN_smaller_kernels.pth")
print("Model with smaller kernels saved.")

# Load the saved model before evaluation
# cnn_model = VGG11_LargerKernels().to(device)  # Ensure to use the correct class name
# cnn_model.load_state_dict(torch.load("CNN_smaller_kernels.pth"))  # Load the saved model
# cnn_model.eval()  # Set the model to evaluation mode
# print("Model with smaller kernels loaded successfully.")
...
plt.title("Confusion Matrix: VGG11 Smaller Kernels")
plt.savefig("CNN_confusion_matrix_smaller_kernels.png")
plt.show()
